...
class UserRepository {
    ...
    async findAll() {
        try {
            const { rows } = await this.db.query(`SELECT u.id, u.name, u.email, u.created_at, u.birthdate, u.gender, u.location, u.latitude, u.longitude, u.last_seen, u.bio, u.photos, u.subscription_tier, u.setup_complete, u.is_deleted,
            ...
            throw new Error(`UserRepository.findAll failed: ${err.message}`);
        }
    }
    async findById(id) {
        try {
            ...
            throw new Error(`UserRepository.findById failed: ${err.message}`);
        }
    }
    async findByEmail(email) {
        try {
            ...
            throw new Error(`UserRepository.findByEmail failed: ${err.message}`);
        }
    }
    async create(data) {
        try {
            ...
            await this.db.execute(`INSERT INTO users (name, email, password_hash, gender, location, bio, photos, subscription_tier)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [data.name, data.email, passwordHash, data.gender ?? null, data.location ?? null, data.bio ?? null, photos, data.subscription_tier ?? "free"]);
            const { rows } = await this.db.query(`SELECT u.id, u.name, u.email, u.created_at, u.birthdate, u.gender, u.location, u.latitude, u.longitude, u.last_seen, u.bio, u.photos, u.subscription_tier
                 FROM users u WHERE u.email = ? LIMIT 1`, [data.email]);
            return rows.map(mapUserRowNoPref)[0];
        }
        catch (err) {
            throw new Error(`UserRepository.create failed: ${err.message}`);
        }
    }
    async updateProfile(id, data) {
        try {
            ...
            if (data.setup_complete !== undefined) {
                fields.push("setup_complete = ?");
                values.push(data.setup_complete ? 1 : 0);
            }
            if (fields.length) {
            ...
            throw new Error(`UserRepository.updateProfile failed: ${err.message}`);
        }
    }
    async updateLocation(id, latitude, longitude) {
        try {
            await this.db.execute(`UPDATE users SET latitude = ?, longitude = ?, last_seen = CURRENT_TIMESTAMP WHERE id = ?`, [latitude, longitude, id]);
            const user = await this.findById(id);
            if (!user)
                throw new Error("User not found");
            return user;
            ...
            throw new Error(`UserRepository.updateLocation failed: ${err.message}`);
        }
    }
    /**
    ...
    async findNearbyForDiscover(params) {
        ...
        `;
        const paramsArr = [latitude, longitude, latitude, userId];
        let whereExtra = "";
        ...
        const { rows } = await this.db.query(sql, paramsArr);
        return rows.map((r) => ({ id: Number(r.id), distance_km: Number(r.distance_km) }));
    }
}
