...
class MatchRepository {
    ...
    async listForUser(userId) {
        try {
            const { rows } = await this.db.query(`SELECT id, user_a_id, user_b_id, is_active, created_at,
            ...
            throw new Error(`MatchRepository.listForUser failed: ${err.message}`);
        }
    }
    async findBetween(a, b) {
        try {
            ...
            const { rows } = await this.db.query(`SELECT id, user_a_id, user_b_id, is_active, created_at,
                        celebration_shown_to_a, celebration_shown_to_b
                 FROM matches WHERE user_a_id = ? AND user_b_id = ? LIMIT 1`, [a1, b1]);
            return rows[0] ?? null;
        }
        catch (err) {
            throw new Error(`MatchRepository.findBetween failed: ${err.message}`);
        }
    }
    async findById(id) {
        try {
            const { rows } = await this.db.query(`SELECT id, user_a_id, user_b_id, is_active, created_at,
                        celebration_shown_to_a, celebration_shown_to_b
                 FROM matches WHERE id = ? LIMIT 1`, [id]);
            return rows[0] ?? null;
        }
        catch (err) {
            throw new Error(`MatchRepository.findById failed: ${err.message}`);
        }
    }
    async createIfMutualLike(a, b) {
        try {
            if (a === b)
                return null;
            const a1 = Math.min(a, b);
            const b1 = Math.max(a, b);
            // Check mutual 'like'
            const { rows: likes } = await this.db.query(`SELECT COUNT(*) as cnt FROM swipes s1
                 JOIN swipes s2 ON s2.swiper_id = ? AND s2.target_id = ? AND s2.direction = 'like'
                 WHERE s1.swiper_id = ? AND s1.target_id = ? AND s1.direction = 'like'`, [b, a, a, b]);
            const isMutual = (likes[0]?.cnt ?? 0) > 0;
            if (!isMutual)
                return null;
            await this.db.execute(`INSERT INTO matches (user_a_id, user_b_id, is_active)
                 VALUES (?, ?, 1)
                 ON DUPLICATE KEY UPDATE is_active = 1, created_at = CURRENT_TIMESTAMP`, [a1, b1]);
            const match = await this.findBetween(a1, b1);
            return match;
        }
        catch (err) {
            throw new Error(`MatchRepository.createIfMutualLike failed: ${err.message}`);
        }
    }
    async deactivate(id) {
        try {
            ...
            throw new Error(`MatchRepository.deactivate failed: ${err.message}`);
        }
    }
    async createMissingForUser(userId) {
        try {
            // Find all mutual likes involving the user that don't have a match row yet
            const { rows: pairs } = await this.db.query(`SELECT DISTINCT pairs.a, pairs.b
                 FROM (
                 ...
                 WHERE m.id IS NULL`, [userId, userId]);
            ...
            for (const p of pairs) {
                await this.db.execute(`INSERT INTO matches (user_a_id, user_b_id, is_active)
                     VALUES (?, ?, 1)
                     ON DUPLICATE KEY UPDATE is_active = 1, created_at = CURRENT_TIMESTAMP`, [p.a, p.b]);
            }
            // Fetch and return the created matches for those pairs
            const createdIdsParams = [];
            const whereClauses = [];
            for (const p of pairs) {
            ...
            const { rows: created } = await this.db.query(sql, createdIdsParams);
            ...
            throw new Error(`MatchRepository.createMissingForUser failed: ${err.message}`);
        }
    }
    async listPendingCelebrations(userId) {
        try {
            ...
            throw new Error(`MatchRepository.listPendingCelebrations failed: ${err.message}`);
        }
    }
    async markCelebrationShown(matchId, userId) {
        try {
            await this.db.execute(`UPDATE matches
                 ...
                 WHERE id = ? AND (user_a_id = ? OR user_b_id = ?)`, [userId, userId, matchId, userId, userId]);
        }
        catch (err) {
            throw new Error(`MatchRepository.markCelebrationShown failed: ${err.message}`);
        }
    }
}
